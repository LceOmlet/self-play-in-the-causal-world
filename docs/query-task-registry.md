...
| `backadj_minimal_sets` | treatment, outcome | one adjustment set | `discovery` | implemented（`task_scoring.py`，到最近合法调整集的图结构编辑距离） |
...
- `backadj` 接收一个完整调整集合；任何满足标准 backdoor criterion 的集合均为合法答案，不要求枚举全部最小集合；
...
decision:
  parsed deployment state -> exact hard-do candidate values
  raw action regret; normalized regret remains a diagnostic

backadj_minimal_sets:
  parsed complete adjustment set -> nearest graphically valid adjustment set
  exact variable-addition/removal distance; valid-set indicator
...
`terminal-quality-v10`，不重新解析模型答案或计算 truth：
...
backadj_minimal_sets:             1 - nearest-valid-set edit distance /
                                  (configured maximum graph nodes - 2)
mediator_set:                     (mediator_f1 + order_f1) / 2
unfinished / illegal terminal:    0
```

实验样本消耗、query 数、轮数与 token 数保持独立诊断，不进入当前训练奖励。
其中 `s` 是固定预算采样分辨率。所有五类 terminal quality 均原样进入 GRPO。
完整合同见 `docs/terminal-quality-reward-v10.md`。
