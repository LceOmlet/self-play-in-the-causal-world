...
\(b_Y\), the combined score direction \(D_Y(x,y)\) from ADR 0021, and the
existing strength draw \(s_Y\sim\operatorname{Uniform}(0,1)\). Let
...
states. The existing parent/state exchangeability, block-energy law, and
strength distribution are unchanged; main effects and interactions are now
score-space effects.
