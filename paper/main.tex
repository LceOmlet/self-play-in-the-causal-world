...
\begin{figure}[t]
  ...
  counterfactual transitions, endpoint marginals supply a Fr\'echet outer
  interval, while the verifier computes the exact interval over complete
  mechanisms compatible with the DAG and every CPT row. A single SCM picks one
  point inside that interval, and an added cross-world assumption selects a
  smaller subset.}
  \label{fig:overview}
\end{figure}
...
marginals leave the overlap between these groups unspecified. Using only these
endpoint marginals, full overlap gives
60 units that change from 0 to 1; zero overlap gives 80. Hence every compatible
coupling satisfies
\[
  P(Y_1{=}1,Y_0{=}0)\in[0.6,0.8],
\]
and both endpoints are attainable by endpoint-event couplings. Constraints from
the complete DAG and all CPT rows can tighten this outer interval. A cross-world
assumption can select a smaller subset; monotonicity $Y_1\ge Y_0$ contracts the
endpoint interval to the point $0.6$.
\end{minipage}}
\end{center}
...
compatible mechanism set and scores either its sharp interval or a submitted
compatible value. The task
...
\cmidrule(lr){1-4}
\textbf{Counterfactual transition} & Units changing outcome across two treatments & sharp interval or compatible scalar & query endpoints read-only \\
\cmidrule(lr){1-4}
...
\begin{equation}
  ...
  \label{eq:cfset}
\end{equation}
The endpoint marginals $p_i=P(Y=y\mid\doop(X=x_i))$ give the Fr\'echet outer
interval $[\max(0,p_1-p_0),\min(p_1,1-p_0)]$. The exact verifier retains all
...
interval mode scores both exact endpoints continuously. The compatible-value
mode accepts any point in the exact interval and reports its distance to it.
...
fixes $P(A=1)=p_1$ and $P(B=1)=p_0$. The desired event is $A=1,B=0$. The two
endpoint marginals alone give the Fr\'echet outer bounds
\begin{equation}
\max(0,p_1-p_0)\le P(A=1,B=0)\le\min(p_1,1-p_0).
\end{equation}
The truth owner instead optimizes this event over complete mechanisms compatible
with the DAG and every CPT row. Its exact interval lies inside the displayed
outer bounds. The interval terminal reports endpoint errors, and the
compatible-scalar terminal reports membership and distance to the exact interval.
Both modes share the same world, query anchors, visible labels, $K$, and $M$.
