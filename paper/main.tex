...
\textbf{Example: interventional marginals leave individual transitions open.}
...
marginals leave the overlap between these groups unspecified. Full overlap gives
60 units that change from 0 to 1; zero overlap gives 80. Hence every compatible
coupling satisfies
\[
  P(Y_1{=}1,Y_0{=}0)\in[0.6,0.8],
\]
and both endpoints are attainable. A cross-world assumption can select a smaller
subset; monotonicity $Y_1\ge Y_0$ contracts the interval to the point $0.6$.
\end{minipage}}
\end{center}

A generated SCM selects one compatible coupling. Encoding that selection as the
label also encodes its cross-world preference. \method{} keeps the full compatible
set and scores either its sharp interval or a submitted compatible value. The task
