...
individual counterfactual probabilities, experimental decisions, causal adjustment sets,
...
action. Adjustment queries ask for a covariate set that removes backdoor bias
without conditioning away the directed effect; discovery queries ask for mediator order after
...
\cmidrule(lr){1-4}
\textbf{Backdoor adjustment} & Covariates that remove backdoor bias while preserving the directed effect & one adjustment set & manipulate and read candidate covariates \\
\cmidrule(lr){1-4}
