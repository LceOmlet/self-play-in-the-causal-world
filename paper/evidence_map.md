...
| The individual counterfactual task asks for one scalar probability conditioned on assigned factual treatment and observed outcome; a hidden sharp compatible interval verifies it. | `individual_counterfactual_probability_bounds` and `validate_individual_counterfactual_probability` in `src/cpt_world/query_truth.py`; scalar parser/scorer; exact bounds tests. | “The verifier accepts scalar probabilities supported by the full set of cross-world couplings compatible with the CPT world.” |
