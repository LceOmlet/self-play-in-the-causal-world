...
def worker(task):
    ...
    with path.open("rb") as f:
        row, world, seed, _cached_truth = pickle.load(f)
    tape_key = f"finite-budget-audit:{seed['seed_id']}:{replicate}"
    # The frozen experiment world is retained, but decision scores must use
    # one current truth owner. Old cached probabilities can differ by an ULP
    # from current inference; mixing them can invalidate normalized regret.
    episode = WorldSpecEpisode(world, seed, OutcomeTape(tape_key))
    ...
    same_passive = score_terminal_answer(
        json.dumps(policy["same_samples_passive_answer"]), seed, world
    )
    result = {
