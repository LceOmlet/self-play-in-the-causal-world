...
def environment(project, run):
    ...
    env.update(
        ...
        CPT_WORLD_PROJECT=str(project),
        CPT_WORLD_RUN_DIR=str(run),
        CPT_WORLD_TRAIN_DATA=str(run / "stream.json"),
...
def pause(args):
    ...
    write(
        args.hold / "initial-preservation.json",
        {
            "time": time.time(),
    ...
    for item in identities:
        try:
            ...
            if (
                abs(member.create_time() - item["create_time"]) < 0.01
...
def prepare(args):
    ...
    write(
        args.run / "stream.json",
        {
            ...
            "stream_start_index": progress["generated_batches"],
            "journal_dir": str(args.run / "task-journal"),
    ...
    allowed.update(
        "ray_kwargs.ray_init.runtime_env.env_vars." + key
    ...
    for key, values in diff.items():
        if (
            isinstance(values["before"], str)
            and isinstance(values["after"], str)
    ...
    dataset = CPTWorldStreamingDataset(
        data_files=[str(args.run / "stream.json")],
        tokenizer=None,
        config=config.data,
    )
...
def launch(args):
    ...
    for identity in read(Path(plan["hold"]) / "stopped.json")["processes"]:
        try:
            process = psutil.Process(identity["pid"])
            assert (
                abs(process.create_time() - identity["create_time"]) >= 0.01
...
def supervise(args):
    plan = read(args.run / "prepared.json")
    process = subprocess.Popen(
        plan["command"], env=environment(args.project, args.run), start_new_session=True
    )
    write(
    ...
    write(
        args.run / "exit.json",
        {"time": time.time(), "exit_code": code, "wall_limit_reached": expired},
    )
    return code
